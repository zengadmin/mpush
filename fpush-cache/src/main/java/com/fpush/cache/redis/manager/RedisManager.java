...
import com.fpush.cache.redis.connection.RedisConnectionFactory;
...
public final class RedisManager implements CacheManager {
	...
	@Override
	public void init() {
		Logs.PUSH.info("begin init redis...");
		factory.setPassword(CC.fp.redis.password);
		factory.setPoolConfig(CC.fp.redis.getPoolConfig(JedisPoolConfig.class));
		factory.setRedisServers(CC.fp.redis.nodes);
		factory.setCluster(CC.fp.redis.isCluster());
		if (CC.fp.redis.isSentinel()) {
			factory.setSentinelMaster(CC.fp.redis.sentinelMaster);
		}
		factory.init();
		test();
		Logs.PUSH.info("init redis success...");
	}
